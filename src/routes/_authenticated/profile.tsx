...
function ProfilePage() {
  ...
  const pillars = [
    ...
    { icon: CalendarCheck, name: "Consistencia", lvl: w.streak, tone: "text-accent", stats: [["Esta semana", `${Math.min(w.runs, RUNS_PER_WEEK)}/${RUNS_PER_WEEK} carreras`], ["Mejor racha", `${w.best} sem`]] },
  ];
  ...
  return (
    <AppShell>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4">
        <h3 className="font-display text-xl tracking-wide text-foreground">Los 4 ámbitos de tu racha</h3>
        {pillars.map((x) => (
